 ...
 * @return array
 *
 * function avasol_body_classes( $classes ) {
 ...
 * return $classes;
 * }
 * add_filter( 'body_class', 'avasol_body_classes' );
 */

/**
 * Remove hentry class
 */
function avasol_remove_hentry( $classes ) {
	$classes = array_diff( $classes, array( 'hentry' ) );

	return $classes;
}

add_filter( 'post_class', 'avasol_remove_hentry' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function avasol_pingback_header() {
	if ( is_singular() && pings_open() ) {
		echo '<link rel="pingback" href="', esc_url( get_bloginfo( 'pingback_url' ) ), '">';
	}
}

add_action( 'wp_head', 'avasol_pingback_header' );

/**
 * Change the default text after an excerpt
 */
function avasol_excerpt_more( $more ) {
	return '...';
}

add_filter( 'excerpt_more', 'avasol_excerpt_more' );

/**
 * Limit the excerpt length
 */
function avasol_excerpt_length( $length ) {
	return 25;
}

add_filter( 'excerpt_length', 'avasol_excerpt_length' );
 ...
 * @return bool
 */
function avasol_menu_has_items( $menu = false ) {
	if ( ! $menu ) {
		return false;
	}
 ...
 */
function avasol_replace_image_lazy( $content ) {
 ...
 * @return mixed
 */
function avasol_image_sizes_js( $response, $attachment, $meta ) {
...
add_filter( 'wp_prepare_attachment_for_js', 'avasol_image_sizes_js', 10, 3 );

/**
 * Bootstrap comment form
 */
function avasol_comment_form( $args ) {
...
add_filter( 'comment_form_defaults', 'avasol_comment_form' );

function avasol_comment_form_fields( $fields ) {
...
add_filter( 'comment_form_default_fields', 'avasol_comment_form_fields' );

/**
 * Tiny MCE Editor changes
 */
/*
function avasol_mce_buttons( $buttons ) {
	array_unshift( $buttons, 'fontselect' ); // Add Font Select
	array_unshift( $buttons, 'fontsizeselect' ); // Add Font Size Select

	return $buttons;
}
add_filter( 'mce_buttons_2', 'avasol_mce_buttons' );

function avasol_mce( $settings ) {
...
add_filter( 'tiny_mce_before_init', 'avasol_mce' );
*/
...
function avasol_yoast_to_bottom() {
	return 'low';
}
add_filter( 'wpseo_metabox_prio', 'avasol_yoast_to_bottom' );
 ...
 */
function avasol_async_scripts($url){
...
add_filter( 'clean_url', 'avasol_async_scripts', 11, 1 );
...
function avasol_admin_style() {
	wp_enqueue_style('admin-styles', get_template_directory_uri().'/css/style-admin.css');
}
	 
add_action('admin_enqueue_scripts', 'avasol_admin_style');
	
function avasol_login_logo() { 
	wp_enqueue_style('admin-login-styles', get_template_directory_uri().'/css/style-login-area.css'); ?>
	<style>
		#login h1:after {
			content: '<?php echo get_bloginfo( 'name' ); ?> admin area'
		}
	</style>
<?php }
	
add_action( 'login_enqueue_scripts', 'avasol_login_logo' );

// disable generated image sizes
function avasol_disable_image_sizes($sizes) {
	...
	unset($sizes['form-edit-image-preview']);    // disable 2x large size
	
	return $sizes;
	
}
add_action('intermediate_image_sizes_advanced', 'avasol_disable_image_sizes');